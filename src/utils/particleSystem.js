...
function particleSystemGenerator(params) {
  const { camera, emitters, parent, rate, texture } = params;
  const uniforms = {
    diffuseTexture: {
  ...
  colorSpline.addPoint(0.0, new THREE.Color(0xf3f70a));
  colorSpline.addPoint(1.0, new THREE.Color(0xf3f70a));
  ...
  const maxLife = 25;
  const maxSize = 50;
  let elapsedTimeAccumulator = 0.0;

  function _AddParticles(timeElapsed) {
    for (let emitter of emitters) {
      elapsedTimeAccumulator += timeElapsed;
      const n = Math.floor(elapsedTimeAccumulator * rate);
      elapsedTimeAccumulator -= n / rate;
      for (let i = 0; i < n; i += 1) {
...
export { particleSystemGenerator };
